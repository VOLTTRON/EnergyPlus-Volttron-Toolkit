...
class GrayBoxZone(object):   
    def __init__(self):
        ...
        self.model = load_fmu(modelName)
        model.set('C',self.c)
        model.set('R',self.r)
        model.set('shgc',self.shgc)
    ...
    def prepareInput(self,q):
        ...
        t_meas = timearr
        u=np.transpose([np.asarray(timearr).flatten(),np.asarray(qsol).flatten(),np.asarray(tout).flatten(),qarr.flatten()])
        return u
